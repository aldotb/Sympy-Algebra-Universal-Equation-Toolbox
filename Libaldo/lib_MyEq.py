...
class MyEq:
    def __init__(self, ksym, name='', var=x, var2=y, var3=z, type='P',vfunc=[], show=True,render=True):
        ...
        self.render=render
        self.bK=[]
        self.ksymt=self.ksym
        if show:
            self.s()
    
    def s(self):
        if self.ksym!=self.ksymt:
            self.bK.append(self.ksymt)
            self.ksymt=self.ksym
    ...
    def undo(self, show=True):
        if len(self.bK) > 0:
            self.ksym = self.bK.pop()
            self.ksymt = self.ksym  # MUY IMPORTANTE
        else:
            print("No hay más undo")
    ...
    def expo(self):
        expr=self.ksym
        if type(expr)==Mul and expr.args[0]==-1:
            return getexpo(expr.args[1])
        else:    
            return getexpo(expr)
    ...
    def base(self):
        expr=self.ksym
        if type(expr)==Mul and expr.args[0]==-1:
            return  getbase(expr.args[1])
        else:    
            return getbase(expr)    


    def apply(self, func, *args, show=True,**kwargs):
        self.ksym = func(self.ksym, *args, **kwargs)    
        if show:
            self.s()
