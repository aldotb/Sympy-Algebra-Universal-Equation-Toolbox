...
def cfrac(*args): # by chatGTP
    """
    Convierte a fracción simbólica o Rational exacto.
    
    Uso:
    cfrac(3,4)       -> 3/4 (Rational)
    cfrac("3/4")     -> 3/4 (Rational)
    cfrac(x,2)       -> x/2 (simbólico)
    cfrac("x/2")     -> x/2 (simbólico)
    cfrac(3,4,'noeval') -> sdiv(3,4) (si tienes sdiv)
    """
    evaluate = True
    vec = []
    ...
    # Filtramos 'noeval'
    for data in args:
        if str(data).lower() in ['noevaluate', 'noeval']:
            evaluate = False
        else:
            vec.append(data)
    ...
    if n == 1:
        ...
        raise ValueError("cfrac solo acepta 1 o 2 argumentos más opcional 'noeval'") 
...
def clearvar(*args):
    ...
    if len(mm)==1:
        ...
        return mm
def diffvar(var): #crea variables dieferenciables
    return get_diff_var(var)
...
def getlatexroot(sexpr):
    ...
    while done:
        ...
        if sexpr[p2:p2+qc]==clave:
            ...
            sres=sexpr[p2+qc:p1]
    kres1=sres
    oldsin='r\\left('+ kres1+'r\\right)^{- \\frac{1}{'+kres2+'}}'
    newsin='r\\sqrt['+kres2+']{'+kres1+' }'
    ...
    while done:
        ...
        if sexpr[p2:p2+qc]==clave:
            ...
            sres=sexpr[p2+qc:p1]
    kres1=sres
    oldsin='r\\left('+ kres1+'\\right)^{\\frac{1}{'+kres2+'}}'
    newsin='r\\sqrt['+kres2+']{'+kres1+' }'
    sexpr=sexpr.replace(oldsin,newsin)
    if 'r\\right)^{- \\frac{1}{' in sexpr or '\\right)^{\\frac{1}{' in sexpr:
