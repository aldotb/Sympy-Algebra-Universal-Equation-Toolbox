...
def sympy2mark(*args, size='Large', copy=True):

    def _latex_clean(expr):
        # Texto
        if isinstance(expr, str):
            if len(expr) == 1:
                return expr
            return r'\mathit{' + expr + '}'
        
        # MyEq
        if hasattr(expr, 'ksym') and hasattr(expr, 'name'):
            return latex(expr.name) + ' = ' + latex(expr.ksym)

        # MyEqEq
        if hasattr(expr, 'e1') and hasattr(expr, 'e2'):
            return latex(expr.e1.ksym) + ' = ' + latex(expr.e2.ksym)

        # Expresión SymPy
        s = latex(expr, mul_symbol='')
        s = regex.sub(r'\^\{([a-zA-Z0-9]+)\}', r'^\1', s)
    ...
    def _convert_obj(obj):
        ...
        if isinstance(obj, tuple):
            if len(obj) == 2:
                return f"{_latex_clean(obj[0])} &= {_latex_clean(obj[1])}"
        
        # Texto
        if isinstance(obj, str):
            return r'\multicolumn{2}{c}{' + _latex_clean(obj) + '}'
        
        # MyEqEq
        if hasattr(obj, 'e1') and hasattr(obj, 'e2'):
            return f"{_latex_clean(obj.e1.ksym)} &= {_latex_clean(obj.e2.ksym)}"
        
        # MyEq
        if hasattr(obj, 'ksym') and hasattr(obj, 'name'):
            return f"{_latex_clean(obj.name)} &= {_latex_clean(obj.ksym)}"
        
        # Expresión sola
        return f"&= {_latex_clean(obj)}"

    # Construir líneas
    lines = []
    for obj in args:
        lines.append(_convert_obj(obj))

    body = r' \\ '.join(lines)

    out = f"$$ \\displaystyle \\{size} \\begin{{aligned}} {body} \\end{{aligned}} $$"
...
class MyEqEq:
    ...
    '''            
    def s(self, *args, kshow=True):  # chatgtp

        # Guardar historial
        if not self.bK1 or self.e1.ksym != self.bK1[-1] or self.e2.ksym != self.bK2[-1]:
            self.bK1.append(self.e1.ksym)
            self.bK2.append(self.e2.ksym)
        ...
        expr = latex_freeze(p1) + ' ' + ps + ' ' + latex_freeze(p2)
    ...
    def apply(self, func, *args, show=True, **kwargs):
        self.e1.apply(func, *args, show=False, **kwargs)
        self.e2.apply(func, *args, show=False, **kwargs)
        if show:
            self.s()
    def dothis(self, func, *args, show=True, **kwargs):
        self.e1.apply(func, *args, show=False, **kwargs)
        self.e2.apply(func, *args, show=False, **kwargs)
        if show:
            self.s()    
    def undo(self, n=1, show=True):
        for _ in range(n):
            if len(self.bK1) > 1:
                self.bK1.pop()
                self.bK2.pop()

        self.e1.ksym = self.bK1[-1]
        self.e2.ksym = self.bK2[-1]
