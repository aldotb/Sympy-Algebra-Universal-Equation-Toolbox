...
def sqrtjoin(expr):
    ...
    # Si es suma -> aplicar a cada término
    if expr.is_Add:
        return Add(*[sqrtjoin(arg) for arg in expr.args])

    # Si es producto -> unir raíces
    if expr.is_Mul:
        coef = 1
        inside = 1
        ...
        if inside != 1:
            return coef * sqrt(inside)
        else:
            return expr

    # Si no es suma ni producto -> dejar igual
    return expr  

# -------------------------------------------------
# SIMPLIFY ROOTS
# sqrt(R^2*(1+cos(a))) -> R*sqrt(1+cos(a))
# -------------------------------------------------

def extract_root(expr):

    if not Is_Root(expr):
        return expr

    inside = insideroot(expr)

    if not Is_Mul(inside):
        return expr

    outside = 1
    inside_new = 1

    for term in inside.args:
        if Is_Pow(term):
            base, exp = term.args
            if exp.is_integer:
                outside *= base**(exp//2)
    ...
    return outside * sqrt(inside_new)    
def rsimplify(expr):
    expr=factor(tsimplify(expand(expr)))
    ...
    if Is_Number(expr):
    ...
    elif Is_Pow(insideroot(expr)) and Is_Root(expr):
        rr,ee=getroot(expr),getpow(insideroot(expr)) 
        if cf(ee,rr)%1==0:
        ...
        if Is_Root(expr):
            return simplify(sqrtjoin(extract_root(expr)))
        else:
            return expr
